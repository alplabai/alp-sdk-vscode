...
function planForHost(
  ...
  const venvPython =
    host === "win32" ? ".venv\\Scripts\\python" : ".venv/bin/python";
  const createVenv =
    host === "win32"
      ? {
          description: "Create local Python venv (.venv)",
  ...
  const upgradePip = {
    description: "Upgrade pip inside .venv",
    command: `${venvPython} -m pip install --upgrade pip`,
  };

  const pythonDeps = {
    description: "Install loader Python deps (.venv)",
    command: `${venvPython} -m pip install pyyaml jsonschema`,
  };

  const west = {
    description: "Install `west` in .venv",
    command: `${venvPython} -m pip install west`,
  };

  if (os === "zephyr") {
    return {
      title: `Bootstrap ALP SDK (Zephyr, ${host})`,
      steps: [createVenv, upgradePip, pythonDeps, west],
      pointers: [
        {
  ...
  if (os === "yocto") {
    ...
    return {
      ...
      steps: [createVenv, upgradePip, pythonDeps, yoctoStep],
      pointers: [
        {
  ...
  return {
    ...
    steps: [createVenv, upgradePip, pythonDeps],
    pointers: [
      {
...
async function pickOs(): Promise<"zephyr" | "yocto" | "baremetal" | null> {
  const items: vscode.QuickPickItem[] = [
    ...
    {
      label: "yocto",
      description: "Yocto / OpenEmbedded user-space (Linux host required)",
    },
    {
      label: "baremetal",
      description: "No OS; vendor toolchains (Alif / Renesas / NXP)",
    },
  ];
