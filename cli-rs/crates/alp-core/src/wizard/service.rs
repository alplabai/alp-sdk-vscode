...
fn gen_c_project_files(def: &WizardTemplateDefinition) -> Vec<WizardPlannedFile> {
    let mut files = vec![
        WizardPlannedFile {
            relative_path: "board.yaml".to_string(),
            content: gen_board_yaml(def),
        },
        WizardPlannedFile {
