...
fn rpmsg_blocked(ch: &IpcChannel, mailbox: &Mailbox) -> bool {
    ch.kind == "rpmsg"
        && matches!(mailbox.controller.as_deref(), None | Some("TBD"))
}
...
fn render_system_ipc_h(board: &BoardYaml, som: &SomPreset) -> String {
    ...
    for ch in &board.ipc {
        ...
        if rpmsg_blocked(ch, &som.mailbox) {
            ...
            lines.push(format!("#define ALP_IPC_{up}_ADDR       0x0u  /* stub: blocked */"));
            lines.push(format!("#define ALP_IPC_{up}_SIZE       0x0u  /* stub: blocked */"));
            lines.push(format!("#define ALP_IPC_{up}_SRC_EPT    0x0u  /* stub: blocked */"));
            lines.push(format!("#define ALP_IPC_{up}_DST_EPT    0x0u  /* stub: blocked */"));
            lines.push(format!("#define ALP_IPC_{up}_MBOX_CH    0u    /* stub: blocked */"));
        }
    }
    lines.push(String::new());
