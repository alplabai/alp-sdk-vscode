...
- Fixed the POSIX shell quoting for streamed runs. Any apostrophe in a binary,
  ...
  matching \`''`.
- `SUPPORTED_CLI_VERSION` is back to `0.3.1`, the latest published tan-cli
  release. Pinned at the unreleased `0.3.2` the download-on-demand asset URL
  404'd, and since a `cached` binary behind the pin also re-fetches, every
  activation retried a download that could not succeed. CI now gates the pin
